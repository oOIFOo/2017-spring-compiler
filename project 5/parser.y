...
%type<exprs> var_ref boolean_expr boolean_term boolean_factor relop_expr expr term factor boolean_expr_list opt_boolean_expr_list condition condition_while
...
program			: ID
			  ...
			  ID
			{
			  ...
			  global_inintial(2, fileName);
			}
			;

program_body		: opt_decl_list {staticvar(symbolTable);}
					opt_func_decl_list
					{
						global_inintial(1, fileName);
					} 
					compound_stmt
			;
...
simple_stmt		: var_ref OP_ASSIGN boolean_expr MK_SEMICOLON
			{
			  ...
			  LHS_access(symbolTable, *$1, *$3, 0);
			}
			| PRINT boolean_expr MK_SEMICOLON { verifyScalarExpr( $2, "print" ); print_code(symbolTable, *$2);}
 			| READ boolean_expr MK_SEMICOLON { verifyScalarExpr( $2, "read" ); read_code(symbolTable, *$2);}
			;
...
while_stmt		: WHILE {fprintf(out_file, "Lbegin_%d:\n", if_id);}
			  condition_while DO {gen_while(symbolTable, *$3, 0);}
			  opt_stmt_list
			  END DO {gen_while(symbolTable, *$3, 1);}
			;

condition_while		: boolean_expr { verifyBooleanExpr( $1, "while" ); $$ = $1;} 
			;
...
for_stmt		: FOR ID 
			{ 
			  ...
			  ptr->stack_id = j_index;
			  j_index++;
			}
			  OP_ASSIGN loop_param TO loop_param
			{
			  verifyLoopParam( $5, $7 );
			  gen_for(symbolTable, $2, $5, $7, 0);
			}
			  DO
			  opt_stmt_list
			  END DO
			{
			  gen_for(symbolTable, $2, $5, $7, 1);
...
factor			: var_ref
			...
			| OP_SUB MK_LPAREN boolean_expr MK_RPAREN
			{
			  ...
			  for(node = $3;node->next != NULL;node = node->next);
			  node->neg_flag = 1;
			}
			...
			| OP_SUB ID MK_LPAREN opt_boolean_expr_list MK_RPAREN
			{
			  ...
			  struct expr_sem *node = (struct expr_sem *)malloc( sizeof(struct expr_sem) );
			  if($4 != NULL){
				for(node = $4;node->next != NULL;node = node->next);
			    node->neg_flag = 1;
			  }
			  else
				$$->neg_flag = 1;
